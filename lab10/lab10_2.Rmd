...
The `fill` argument is straightforward; we are filling in each box by class. What does `color` do?  
```{r}
homerange %>% 
...
`group` can work the same way as fill, but it does not color code the boxes.  
```{r}
homerange %>% 
